...
interface TextInputWithIconProps extends React.InputHTMLAttributes<HTMLInputElement> {
    icon?: React.ReactNode;
    containerClassName?: string;
}

export function TextInputWithIcon({
    icon = <DefaultVoucherIcon />,
    containerClassName = "",
    className = "",
    ...
    return (
        <div
            className={`flex w-full items-center gap-2 px-3 py-3 border border-primary-orange rounded-3xl ${containerClassName}`}
        >
            {icon}
            <input
                type="text"
                ...
                className={`
                    flex-1 text-sm bg-transparent border-none focus:outline-none focus:ring-0 
                    text-title-black
                    placeholder:text-body-grey placeholder:capitalize 
                    ${className}
                `}
                {...props}
            />
